...
├── eval_gate.py             — The Arena: pits challenger models against @production champ
...
[milestone] — DAYS 1-3 (data + baseline + DistilBERT) COMPLETE for both languages.

[decision] — Days 4-6 backend implementation order strictly locked to enforce separation of concerns and deployment readiness: 1) config.py (Environment), 2) schemas.py (Data Contract), 3) models.py & database.py (Postgres/Redis Vault), 4) risk_engine.py (Business Logic Brain), 5) main.py (FastAPI Traffic Cop).
...
[milestone] — DAYS 4-6 COMPLETE. FastAPI backend built with dynamic Pydantic data validation. SQLAlchemy + PostgreSQL database (The Vault) connected via Docker. SentinelRiskEngine implemented for context-aware risk scaling. langdetect + heuristic router successfully routing traffic between English and Hinglish models.
...
THREAT corpus (Hammer et al. 2019) — English, violent-threat-specific.
...
Weakly-labeled Hinglish dataset (built by us) — since every pre-packaged Hinglish
